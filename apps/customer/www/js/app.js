...
class HomepageView {
    static async render() {
        return VerticalLayout(
            H1('Welcome to ERP'),
            Paragraph('This is the Customer portal homepage.'),
            Card(
                H2('Overview'),
                Paragraph('Your account is active and connected.')
            )
        ).addClass('page');
    }
}
    ...
    static async render() {
        ...
        const saveBtn = new Button('Save Server Config', async () => {
            ...
            if (url && (url.startsWith('http://') || url.startsWith('https://'))) {
                ...
                await NativeService.showToast('Server config updated');
        ...
        return VerticalLayout(
            ...
            Paragraph('Server IP Configuration:'),
            HorizontalLayout(serverInput, saveBtn),
            new Button('Check for Updates', () => window.otaClient?.check(false), 'outline')
        ).addClass('page');
    }
}
