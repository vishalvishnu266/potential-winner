/**
 * Persistent bottom tab bar. Rebuilds only the button set on route change
 * (via `.updateActive`), never the whole DOM node — so scroll position of
 * the outer app is untouched.
 */
...
export function TabBar(labels: {
  ...
  for (const t of tabs) {
    ...
    btn.add(
      El('span').cls('tab-ico').add(Icon(t.icon, { size: t.emphasise ? 22 : 22 })),
      t.emphasise ? null : El('span').text(t.label),
    );
    btn.onClick(() => { if (router.currentPath !== t.path) router.navigate(t.path); });
    btns.set(t.name, btn);
    root.add(btn);
  }
